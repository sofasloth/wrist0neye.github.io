...
그럼에도 불구하고 여전히 동작하지 않는다. 결국 직접 만들어야 하는 상황이 왔는데 이 때 [regex simulator for ruby](https://rubular.com/)랑 [ruby의 sub, gsub]() 설명 글을 참고하면 좋다.
...
![](Pasted%20image%2020240811163621.png)
이런 원인이 발생한 이유는 `<a>` 태그에 `href` 속성이 없는 경우 발생한다고 하는데 내가 허접하게 만든 custom linkpreview로 인해 발생한 것으로 추측된다. 이를 보완하기 위해 `href` 값이 없는 경우 `href = "#"`으로 채우게 만들어야 한다. 따라서 `linkpreview.html` 파일에 **링크가 없을 때는 `#`을 채우는 `\{% assign og_url = url | default : "#" %\}`** 코드를 추가해주면 된다.
...
<div class="card bg-transparent jekyll-linkpreview-wrapper">
  {% assign og_url = url | default = "#" %}
  <a href="{{og_url}}" class="post-preview jekyll-linkpreview-linkbox"></a>
    <!-- 생략 -->
```
{% endraw %}

그리고 *Chirpy* 블로그 자체도 조금 `a` 태그 failure 자주 발생하는데, 이에 대해서 해결방안은 아래 링크에서 참고하면 된다.

<% linkpreview "https://github.com/cotes2020/jekyll-theme-chirpy/issues/1403" %>
▲ 블로그 제작자가 직접 답글 단 Github Issue 문항
