...
#### 아래 relative-link 구현하는 용도로 만든 dummy header 입니다. 특수문자 1!2@3#4$5%6^7&8*9.0,a;b c<>d e{} 어떻게 보일까요?  (참조)
...
또, `[[#]]`으로 본문의 heading을 참조해야 하는데, 이 때는 아래 사진처럼 띄어쓰기나 `&` 문자는 `-`로 치환해서 참조해야 한다.

![](/assets/img/res/Pasted%20image%2020240811042940.png)


일단 obsidian의 규칙을 좀 더 파악해보자.
```md
[아래 relative-link 구현하는 용도로 만든 dummy header 입니다. 특수문자 1!2@3#4$5%6^7&8*9.0,a;b c<>d e{} 어떻게 보일까요?  (참조)](#아래%20relative-link%20구현하는%20용도로%20만든%20dummy%20header%20입니다.%20특수문자%201!2@3%204$5%6%207&8*9.0,a;b%20c<>d%20e{}%20어떻게%20보일까요?%20(참조))
```
...
> - 옵시디언의 경우 제목에 오는 # 문자는 `%20`으로 치환되는데 이건 따로 구분할 방법이 없다. 최대한 Heading에 `#` 외에도 `^, [], |` 문자를 섞어쓰지 않도록 주의한다.
> 	- ![](/assets/img/res/Pasted%20image%2020240811043914.png)
{:prompt-danger}

### [2. linkpreview (richlink)](https://github.com/ysk24ok/jekyll-linkpreview)
```ruby
...
#### linkpreview 결과물
아직은 최적화가 덜 되었는데 추후에 시간되면 추가 보완할 예정이다.
