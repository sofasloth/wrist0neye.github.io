...
`()` 안의 문자열을 어떻게 수정할지만 생각해보자. 변환 순서를 고려해보면 아래처럼 나타낼 수 있을 것이다.
1. `()`안에서 처음 시작하는 문자가 `http`를 포함하면 안 된다.
...
그럼에도 불구하고 여전히 동작하지 않는다. 어차피 Github Action은 보안문제로 커스텀 plugin이 있으면 빌드취소하고 에러를 뱉는다. 결국 직접 만들어야 하는데, 파이썬 스크립트로 
